...
type LabelMode = 'do-it' | 'pass' | null;

// ========== STATE ==========
const INITIAL_WEIGHTS: number[] = [0.3, -0.5, -0.4, 0.2, 0.1];
const WEIGHT_NAMES: string[] = ['Fun Factor', 'Danger Level', 'Trouble Risk', 'Long-term Benefit', 'Peer Pressure'];
const LEARNING_RATE: number = 0.1;

let weights: number[] = [...INITIAL_WEIGHTS];
let bias: number = 0;
let trainingPoints: TrainingPoint[] = [];
let stepCount: number = 0;
let labelMode: LabelMode = null;
let isTraining: boolean = false;
let prevWeights: number[] = [...INITIAL_WEIGHTS];
let prevBias: number = 0;

// DOM refs
const funSlider = document.getElementById('funSlider') as HTMLInputElement;
const dangerSlider = document.getElementById('dangerSlider') as HTMLInputElement;
const troubleSlider = document.getElementById('troubleSlider') as HTMLInputElement;
const benefitSlider = document.getElementById('benefitSlider') as HTMLInputElement;
const pressureSlider = document.getElementById('pressureSlider') as HTMLInputElement;
const biasSlider = document.getElementById('biasSlider') as HTMLInputElement;

const funVal = document.getElementById('funVal') as HTMLSpanElement;
const dangerVal = document.getElementById('dangerVal') as HTMLSpanElement;
const troubleVal = document.getElementById('troubleVal') as HTMLSpanElement;
const benefitVal = document.getElementById('benefitVal') as HTMLSpanElement;
const pressureVal = document.getElementById('pressureVal') as HTMLSpanElement;
const biasVal = document.getElementById('biasVal') as HTMLSpanElement;

const decisionOutput = document.getElementById('decisionOutput') as HTMLDivElement;
const decisionText = document.getElementById('decisionText') as HTMLDivElement;
const decisionConfidence = document.getElementById('decisionConfidence') as HTMLDivElement;
const confidenceLabel = document.getElementById('confidenceLabel') as HTMLDivElement;
const mathBreakdown = document.getElementById('mathBreakdown') as HTMLDivElement;

const canvas = document.getElementById('vizCanvas') as HTMLCanvasElement;
const ctx = canvas.getContext('2d')!;
const canvasTooltip = document.getElementById('canvasTooltip') as HTMLDivElement;

const labelModeInfo = document.getElementById('labelModeInfo') as HTMLDivElement;
const trainProgress = document.getElementById('trainProgress') as HTMLDivElement;
const trainProgressText = document.getElementById('trainProgressText') as HTMLSpanElement;
const trainProgressBar = document.getElementById('trainProgressBar') as HTMLDivElement;
...
function calculateZ(inputs: number[], w: number[], b: number): number {
  let z = b;
  for (let i = 0; i < inputs.length; i++) z += inputs[i] * w[i];
  return z;
}

function predict(inputs: number[], w: number[], b: number): number {
...
function update(): void {
  ...
  if (output > 0.9) confidenceLabel.textContent = 'Highly confident';
  else if (output > 0.7) confidenceLabel.textContent = 'Fairly confident';
  else if (output > 0.55) confidenceLabel.textContent = 'Leaning towards it';
  else if (output > 0.45) confidenceLabel.textContent = 'Uncertain';
  else if (output > 0.3) confidenceLabel.textContent = 'Leaning away';
  else if (output > 0.1) confidenceLabel.textContent = 'Fairly confident';
  else confidenceLabel.textContent = 'Highly confident';
...
function updateWeightsDisplay(): void {
...
function toCanvasX(funVal: number): number {
  const w = canvas.width / window.devicePixelRatio;
  return PLOT_PADDING + (funVal / 10) * (w - 2 * PLOT_PADDING);
}

function toCanvasY(dangerVal: number): number {
...
function drawVisualization(): void {
  ...
  ctx.lineWidth = 1;
  ctx.globalAlpha = 0.5;
  ctx.beginPath(); ctx.moveTo(curX, plotTop); ctx.lineTo(curX, plotTop + plotHeight); ctx.stroke();
  ctx.beginPath(); ctx.moveTo(plotLeft, curY); ctx.lineTo(plotLeft + plotWidth, curY); ctx.stroke();
  ctx.globalAlpha = 1;
...
function updateStats(): void {
  ...
  for (const pt of trainingPoints) {
    ...
    if (predictedLabel === pt.label) correct++;
...
async function runTraining(iterations: number = 100): Promise<void> {
  if (trainingPoints.length === 0 || isTraining) return;
