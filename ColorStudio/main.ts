...
function getColorName(rgb: RGB): string {
  ...
  if (s < 0.08) return l < 0.45 ? "Dark Gray" : "Light Gray";
  for (const band of HUE_BANDS) {
    if (h <= band.maxHue) return band.name;
  }
  return "Red";
}
...
function shiftHsl(base: HSL, dh: number, ds = 0, dl = 0): RGB {
  return hslToRgb({
    h: wrapHue(base.h + dh),
...
function generateHarmony(base: RGB, mode: HarmonyMode): RGB[] {
...
// Vienot et al. LMS-based matrices
type Matrix3x3 = [
...
const CB_MATRICES: Record<Exclude<VisionMode, "normal">, Matrix3x3> = {
  protanopia: [
    [0.567, 0.433, 0.0],
    [0.558, 0.442, 0.0],
    [0.0, 0.242, 0.758],
...
class ColorStudio {
  ...
  private g = 100;
  private b = 50;
  private harmony: HarmonyMode = "complementary";
  private vision: VisionMode = "normal";
  private sloshTimerId = 0;
  private toastTimerId = 0;

  // DOM refs — typed and non-null asserted after bindDomRefs()
  private sliderR!: HTMLInputElement;
  private sliderG!: HTMLInputElement;
  private sliderB!: HTMLInputElement;
  private valR!: HTMLElement;
  private valG!: HTMLElement;
  private valB!: HTMLElement;
  private streamR!: HTMLElement;
  private streamG!: HTMLElement;
  private streamB!: HTMLElement;
  private bowlFill!: HTMLElement;
  private readoutSwatch!: HTMLElement;
  private readoutHex!: HTMLElement;
  private readoutRgb!: HTMLElement;
  private readoutName!: HTMLElement;
  private baseSwatch!: HTMLElement;
  private paletteContainer!: HTMLElement;
  private cbPaletteContainer!: HTMLElement;
  ...
  private bindDomRefs(): void {
    this.sliderR = this.qs<HTMLInputElement>("#slider-r");
    this.sliderG = this.qs<HTMLInputElement>("#slider-g");
    this.sliderB = this.qs<HTMLInputElement>("#slider-b");
    ...
    this.randomizeBtn = this.qs<HTMLButtonElement>("#randomize-btn");
  ...
  private bindEvents(): void {
    ...
    document
      .querySelectorAll<HTMLButtonElement>(".harmony-btn")
      .forEach((btn) => {
        btn.addEventListener("click", () => {
          this.harmony = btn.dataset.mode as HarmonyMode;
          document
    ...
    document
      .querySelectorAll<HTMLButtonElement>(".cb-btn")
      .forEach((btn) => {
        btn.addEventListener("click", () => {
          this.vision = btn.dataset.vision as VisionMode;
          document
  ...
  private render(): void {
    const rgb: RGB = { r: this.r, g: this.g, b: this.b };
  ...
  private updateStream(el: HTMLElement, value: number): void {
  ...
  private triggerSlosh(): void {
    ...
    this.sloshTimerId = window.setTimeout(
      () => this.bowlFill.classList.remove("sloshing"),
      600
    );
  }
  ...
  private renderCbPalette(): void {
    const colors = generateHarmony(
      { r: this.r, g: this.g, b: this.b },
      this.harmony
    ).map((c) => simulateVision(c, this.vision));
  ...
  private qs<T extends Element = HTMLElement>(selector: string): T {
