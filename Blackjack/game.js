...
function showResult(message, type) {
    ...
    if (type === 'win')
        elPlayerZone.classList.add('win-glow');
    else if (type === 'lose') {
        ...
        elPlayerZone.classList.add('push-glow');
    else if (type === 'blackjack')
        elPlayerZone.classList.add('bj-flash');
}
// =============================================
// DEALING WITH ANIMATION
// =============================================
async function dealCardAnimated(hand, container, faceUp = true) {
...
async function nextHandOrDealerTurn() {
    ...
    const nextIndex = state.playerHands.findIndex((h, i) => i > state.activeHandIndex && !h.hasStood && h.cards.length > 0);
    if (nextIndex !== -1) {
...
function onChipClick(value) {
    if (state.currentBet + value > state.balance)
        return;
...
function getJackAdvice() {
    ...
    if (canSplitNow) {
        ...
        if (r === 'A')
            return { action: 'Split', quip: "Always split aces. Always. Don't make me say it twice." };
        if (pv === 8)
            return { action: 'Split', quip: "Eights. You split them. No exceptions — that's the code." };
        if (pv === 10)
            return { action: 'Stand', quip: "Twenty. Nobody splits a winning hand. Walk away." };
        if (pv === 5) {
    ...
    if (isSoft) {
        if (value >= 19)
            return { action: 'Stand', quip: "Soft nineteen or better. You stand. This isn't debatable." };
        if (value === 18) {
    ...
    // ——— HARD HANDS ———
    if (value >= 17)
        return { action: 'Stand', quip: "Seventeen or better. You stand. That's not advice — that's law." };
...
function showJackBubble(action, quip, cost) {
    if (jackBubbleTimeout)
        clearTimeout(jackBubbleTimeout);
